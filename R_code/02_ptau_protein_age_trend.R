###############################################################################
# 02_ptau_protein_age_trend.R
# Age-trend tracking analysis: which proteins' age-trajectory best matches PTAU?
# - LOESS smoothing of log2(PTAU) ~ AGE and log2(protein) ~ AGE
# - Spearman correlation of LOESS fitted values = trend similarity score
# - Rank proteins by this score; plot top 10 with PTAU
###############################################################################
...
LOESS_SPAN      <- 0.75   # default span for LOESS smoothing
MIN_VALID_PAIRS <- 30     # minimum valid (AGE, protein) pairs to fit LOESS
TOP_N_PLOT      <- 10     # how many top proteins in the combined figure
TOP_N_EXPORT    <- 20     # how many top proteins in CSV/individual plots

# Output directory
out_dir <- "output/ptau_protein_age_trend"
...
# ===========================================================================
# 2. Filter to baseline, non-missing PTAU, non-missing AGE
# ===========================================================================
cat("\n========== Filtering samples ==========\n")
...
# Filter to non-missing PTAU AND non-missing AGE
...
# Count "NA" strings in a sample of columns
...
# ===========================================================================
# 8. Per-protein LOESS and trend correlation (core loop)
# ===========================================================================
...
results <- data.frame(
  ...
  n_valid         = NA_integer_,
  loess_corr      = NA_real_,       # signed Spearman rho (PTAU fitted vs protein fitted)
  loess_corr_abs  = NA_real_,       # absolute Spearman rho
  loess_mae_z     = NA_real_,       # z-scored mean absolute error
...
for (i in seq_len(n_proteins)) {
  ...
  if (n_valid >= MIN_VALID_PAIRS) {
    ...
    if (!is.null(loess_fit)) {
      ...
      # Get protein fitted values
      prot_fitted <- rep(NA_real_, length(prot_log2))
      prot_fitted[valid_idx] <- fitted(loess_fit)

      # Spearman correlation: PTAU fitted vs protein fitted (shared valid points)
      ...
      # Z-score MAE: mean absolute error between z-scored fitted curves
...
# Separate successfully ranked vs failed
...
if (nrow(results_failed) > 0) {
  ...
  n_no_cor     <- sum(results_failed$loess_converged & is.na(results_failed$loess_corr), na.rm = TRUE)
  cat(sprintf("  Insufficient valid pairs (< %d): %d\n", MIN_VALID_PAIRS, n_low_n))
  cat(sprintf("  LOESS failed to converge: %d\n", n_no_loess))
  cat(sprintf("  Spearman correlation failed: %d\n", n_no_cor))
}

# Sort by signed loess_corr descending (highest = best tracks PTAU direction)
...
cat(sprintf("loess_mae_z distribution:\n"))
cat(sprintf("  Min:    %.4f\n", min(results_valid$loess_mae_z, na.rm = TRUE)))
cat(sprintf("  Median: %.4f\n", median(results_valid$loess_mae_z, na.rm = TRUE)))
cat(sprintf("  Max:    %.4f\n", max(results_valid$loess_mae_z, na.rm = TRUE)))
...
# Create lookup: Analytes column has "X10000.28" format matching protein column names
...
# Re-sort after merge (merge may shuffle)
results_valid <- results_valid[order(results_valid$loess_corr, decreasing = TRUE), ]
results_valid$rank <- seq_len(nrow(results_valid))
...
get_gene_label <- function(df) {
  ...
  # If same gene appears multiple times, append protein_id
...
# ===========================================================================
cat("\n========== Top 20 Proteins (best tracking PTAU age-trend) ==========\n")
...
# Also: worst tracking (most negative)
...
write.csv(results_valid,
          file.path(out_dir, "all_proteins_age_trend_ranking.csv"),
          row.names = FALSE)
cat(sprintf("Saved: all_proteins_age_trend_ranking.csv (%d rows)\n", nrow(results_valid)))

# Top N export
write.csv(head(results_valid, TOP_N_EXPORT),
          file.path(out_dir, "top20_best_tracking_proteins.csv"),
          row.names = FALSE)
cat(sprintf("Saved: top20_best_tracking_proteins.csv\n"))
...
if (nrow(results_failed) > 0) {
  ...
  write.csv(results_failed,
            file.path(out_dir, "removed_proteins_loess_failure.csv"),
            row.names = FALSE)
  cat(sprintf("Saved: removed_proteins_loess_failure.csv (%d rows)\n", nrow(results_failed)))
}

# ===========================================================================
# 14. Combined plot: PTAU + top 10 proteins (z-score normalized)
# ===========================================================================
...
# Build plot data: one row per subject per "series"
# PTAU scatter (z-scored raw values, not fitted)
...
# Ensure PTAU is first in legend
plot_lines$series <- factor(plot_lines$series,
                            levels = unique(plot_lines$series))

# Color palette: PTAU = black, proteins = color
n_prot_lines <- length(unique(plot_lines$series)) - 1  # minus PTAU
if (n_prot_lines > 0) {
...
# Line widths: PTAU thicker
plot_lines$linewidth <- ifelse(plot_lines$series == "PTAU", 1.4, 0.8)

# Legend labels with correlation values
legend_labels <- setNames(
  as.character(plot_lines$series),
  plot_lines$series
)
# For protein lines, append rho value
...
combined_p <- ggplot() +
  # PTAU scatter (z-scored raw values)
  ...
  labs(
    title = "Proteins Whose Age-Trajectory Best Tracks PTAU",
    subtitle = sprintf(
      "LOESS span = %.2f | Z-scored fitted values | %d baseline subjects | Top %d of %d proteins",
...
# Save
ggsave(file.path(out_dir, "combined_ptau_top10_age_trend.pdf"),
       combined_p, width = 10, height = 7, device = "pdf")
ggsave(file.path(out_dir, "combined_ptau_top10_age_trend.png"),
       combined_p, width = 10, height = 7, dpi = 300, device = "png")
cat("Saved: combined_ptau_top10_age_trend.pdf/png\n")
...
for (k in seq_len(min(TOP_N_PLOT, nrow(top_plot)))) {
  ...
  # Build data frame for this protein
  plot_df <- data.frame(
    AGE           = age_vals,
  ...
  # Z-score for overlaid PTAU trend
  ptau_display_z <- ptau_fitted_z

  # Protein z-scored fitted
  prot_display_z <- (prot_fitted - mean(prot_fitted, na.rm = TRUE)) /
                    sd(prot_fitted, na.rm = TRUE)

  # Build combined trend comparison data
  trend_df <- data.frame(
    AGE     = rep(age_vals, 2),
    fitted_z = c(ptau_display_z, prot_display_z),
    Series   = rep(c("PTAU", gene_lbl), each = length(age_vals))
  )
  ...
  p <- ggplot() +
    ...
    labs(
      ...
      subtitle = sprintf(
        "Trend corr ρ = %.4f | MAE_z = %.4f | N = %d | %s",
  ...
  ggsave(file.path(out_dir, sprintf("indiv_trend_%02d_%s.pdf", k, safe_name)),
         p, width = 7, height = 5, device = "pdf")
  ggsave(file.path(out_dir, sprintf("indiv_trend_%02d_%s.png", k, safe_name)),
         p, width = 7, height = 5, dpi = 300, device = "png")
}

# Combined multi-panel figure of all top individual plots
if (length(indiv_plots) > 0) {
  # Use a 5x2 or 3x4 layout depending on count
  ...
  multi_p <- wrap_plots(indiv_plots, ncol = ncol) +
    plot_annotation(
      title   = "Top Proteins Tracking PTAU Age-Trend (Individual Views)",
      subtitle = sprintf("log2(protein) vs AGE with LOESS trend | %d baseline subjects",
                         nrow(df_analysis)),
  ...
  ggsave(file.path(out_dir, "indiv_trends_multi_panel.pdf"),
         multi_p, width = multi_w, height = multi_h, device = "pdf",
         limitsize = FALSE)
  ggsave(file.path(out_dir, "indiv_trends_multi_panel.png"),
