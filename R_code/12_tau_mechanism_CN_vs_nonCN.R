...
run_tau_mechanism <- function(df_grp, grp_label) {
  ...
  # Correlation
  df_grp$PTAU_log2 <- log2(df_grp$PTAU)
  ...
  results <- merge(results, dict[,c("Analytes","EntrezGeneSymbol")], by.x="protein_id", by.y="Analytes", all.x=TRUE)
...
# ---- 2. Run both groups ----
df_cn <- df_bl[df_bl$DX == "CN", ]
df_noncn <- df_bl[df_bl$DX %in% c("EMCI", "LMCI", "AD"), ]
